...
def check_data(f, n):
    ...
    elif 'supermasterflat_link' not in t:
        raise RuntimeError("Flat images not found in {}".format(f))
    elif 'bias_no_cal' not in t:
    ...
    if not os.path.exists(f+n+'/raw_no_cal/darks_no_cal'):
    ...
    for f in fl:
        if (os.path.exists(f+field_name) and
           (not check_calibration(f+field_name) or rerun == 1)):
            ...
            print 'Calibration of {} successfully completed.\n'.format(f[-9:-1])
